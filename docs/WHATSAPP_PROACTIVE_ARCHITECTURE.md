...
Normal Brain replies now return `assistant_message_id`. After physical delivery, the checked-in bridge calls the existing outbox endpoint with `action=record_reply_delivery`. Because `client.sendMessage()` may return no message object despite delivery, both normal and proactive sends capture the matching outgoing `message_create` event as a fallback. Proactive sent ACKs include `delivery_attempt` and all validated outgoing provider IDs. The backend rejects sent ACKs with no provider identity and stores one row per physical WhatsApp bubble before reporting mapping success, so quoting any chunk resolves to the same logical assistant/outbox target. Singular ID fields remain backward compatible. See [WHATSAPP_BRIDGE_RELIABILITY_PATCH.md](WHATSAPP_BRIDGE_RELIABILITY_PATCH.md).
