...
Live QA checkpoint, 2026-09-26: the deployed MCP read smoke and app explicit remember/reconfirm worked; `search_memory` and shared-context autobiography were available. The physical WhatsApp inbound/reply round trip worked, confirmed by the user. Do not mark the full journey green: a recall question wrote an unintended memory, a punctuation-only cross-channel restatement duplicated an active memory, and a dated event was classified as a semantic fact. The local OAuth link secret failed authorize with HTTP 401, so live code replay and write-scope sync were not tested. No production project exists for positive grounding QA; a later `Thanks` test hit Gemini 503. Details are in `docs/CODEX_COMPANION_VNEXT_SLICE3_PROGRESS.md`.
